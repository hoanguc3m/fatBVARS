...
#'       V_Zt <- solve(V_Zt_inv)
#'       mu_Zt <- as.vector(V_Zt %*% D %*% (Sigma2_inv /w_sample[i]) %*% (yt[,i] - B %*% xt[,i]) )
...
BVAR.Hyper.Student.novol <- function(y, K, p, y0 = NULL, prior = NULL, inits = NULL){
  ...
  for (j in c(1:samples)){
    ...
    for (i in c(1:t_max)){
      ran_stu = rt(1, df = 4)
      a_eq <- -0.5* (t(gamma) %*% Sigma2_inv %*% gamma)
      b_eq <- -((0.5*K + nu/2+1))
      c_eq =  0.5 * t(u[,i]) %*% Sigma2_inv %*% u[,i] + nu/2
      ...
      num_mh =  dinvgamma(w_temp, shape = nu*0.5, rate = nu*0.5, log = T) +  # prior
        ( - 0.5 * K * log_w_temp - 0.5 * t((u[,i] - gamma*w_temp)) %*% Sigma2_inv %*% (u[,i] - gamma*w_temp) / w_temp) - # posterior
      ...
      denum_mh = dinvgamma(w_sample[i], shape = nu*0.5, rate = nu*0.5, log = T) +  # prior
        ( - 0.5 * K * log(w_sample[i]) - 0.5 * t((u[,i] - gamma*w_sample[i])) %*% Sigma2_inv %*% (u[,i] - gamma*w_sample[i]) / w_sample[i]) - # posterior
...
BVAR.multiStudent.novol <- function(y, K, p, y0 = NULL, prior = NULL, inits = NULL){
  ...
  B <- Vec_to_Mat(inits$B0, K, p)
  sigma <- inits$sigma
  Sigma <- solve(inits$A0) %*% diag(sigma)
  Sigma2 <- Sigma %*% t(Sigma)
  Sigma2_inv <- solve(Sigma2)
  V_b_prior_inv <- solve(V_b_prior)
  ...
  for (j in c(1:samples)){
    ...
    for (i in c(1:t_max)){
      V_b_post_inv <- V_b_post_inv + kronecker(xt[,i] %*% t(xt[,i]), diag(w_sqrt_inv[,i]) %*% Sigma2_inv %*% diag(w_sqrt_inv[,i]) )
      b_post <- b_post + kronecker(xt[,i], (diag(w_sqrt_inv[,i]) %*% Sigma2_inv %*% diag(w_sqrt_inv[,i])) %*% yt[,i])
    }
    ...
    for (i in c(1:t_max)){
      ...
      b_target <- (nu*0.5 + 0.5 * u_proposal[,i]^2 / sigma^2) * 0.75 # adjust by 0.75
      w_temp = w[,i]
      for (k in c(1:K)){
...
BVAR.Hyper.multiStudent.novol <- function(y, K, p, y0 = NULL, prior = NULL, inits = NULL){
  ...
  for (j in c(1:samples)){
    ...
    for (i in c(1:t_max)){
      V_b_post_inv <- V_b_post_inv + kronecker(xt[,i] %*% t(xt[,i]), diag(w_sqrt_inv[,i]) %*% Sigma2_inv %*% diag(w_sqrt_inv[,i]) )
      b_post <- b_post + kronecker(xt[,i], (diag(w_sqrt_inv[,i]) %*% Sigma2_inv %*% diag(w_sqrt_inv[,i])) %*% (yt[,i] - gamma * w[,i]))
    }
    ...
    for (i in c(1:t_max)){
      V_gamma_post_inv <- V_gamma_post_inv +  diag(w_sqrt[,i]) %*% Sigma2_inv %*% diag(w_sqrt[,i]) # ww' sqrt(w_inv) Signma_inv sqrt(w_inv)
      gamma_post <- gamma_post + (diag(w_sqrt[,i]) %*% Sigma2_inv %*% diag(w_sqrt_inv[,i])) %*% (yt[,i] - B %*% xt[,i] )
    }
    ...
    u_proposal <- A %*% (yt - B %*%xt)
    for (i in c(1:t_max)){
      a_target <- (nu*0.5 + 1*0.5) * 0.7
      b_target <- (nu*0.5 + 0.5 * u_proposal[,i]^2 / sigma^2)*0.7  # adjust by 0.75
      w_temp = w[,i]
      for (k in c(1:K)){
        w_temp[k] <- rinvgamma(1, shape = a_target[k], rate = b_target[k])
      }
      ...
      num_mh =  sum(dinvgamma(w_temp, shape = nu*0.5, rate = nu*0.5, log = T)) +  # prior
        ( - 0.5 * sum(log_w_temp) - 0.5 * t(u[,i] - gamma*w_temp) %*% diag(w_temp_sqrt_inv) %*% Sigma2_inv %*% diag(w_temp_sqrt_inv) %*% (u[,i] - gamma*w_temp)) - # posterior
        sum( dinvgamma(w_temp, shape = a_target, rate = b_target, log = T)) # proposal
      denum_mh = sum(dinvgamma(w[,i], shape = nu*0.5, rate = nu*0.5, log = T)) + # prior
        (-0.5) * sum(log(w[,i])) - 0.5 * t(u[,i] - gamma*w[,i]) %*% diag(w_sqrt_inv[,i]) %*% Sigma2_inv %*% diag(w_sqrt_inv[,i]) %*% (u[,i] - gamma*w[,i])  - # posterior
        sum( dinvgamma(w[,i], shape = a_target, rate = b_target, log = T))
      alpha = num_mh - denum_mh;
      temp = log(runif(1))
      if (alpha > temp){
